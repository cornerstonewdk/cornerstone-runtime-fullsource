...
{
	self = (RTFile*)[super initWithWebView:theWebView];
	if(self)
	{
...
- (void) requestFileSystem:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (type > 1){
		result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt: NOT_FOUND_ERR];
		...
		if (pNumAvail && [pNumAvail unsignedLongLongValue] < size) {
			result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt: QUOTA_EXCEEDED_ERR];
			jsString = [result toErrorCallbackString:callbackId];
		} 
		else {
			...
			result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsDictionary: fileSystem];
...
- (void) resolveLocalFileSystemURI:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	NSURL* desturi = [self getURIfromString:[arguments objectAtIndex:0]];
	RTPluginResult* result = nil;
	
	if (!desturi || ![desturi isFileURL]) {
		// issue ENCODING_ERR
		result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt: ENCODING_ERR];
		jsString = [result toErrorCallbackString:callbackId];
	} else {
        NSFileManager* fileMgr = [[NSFileManager alloc] init];
		NSString* path = [desturi path];
        NSLog(@"url path: %@", path);
        BOOL	isDir = NO;
		...
		if (bExists) {
			...
			if (foundFullPath == nil) {
				...
				result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt: SECURITY_ERR];
				jsString = [result toErrorCallbackString:callbackId];
			} else {
...
- (void) getFile: (NSMutableArray*) arguments withDict:(NSMutableDictionary*)options
{
	...
	if ([requestedPath rangeOfString: @":"].location != NSNotFound) {
		...
		if (bExists && create == NO && bIsDir == !bDirRequest) {
			...
			if(!bSuccess){
				...
				result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsDictionary: [self getDirectoryEntry: reqFullPath isDirectory: bDirRequest]];
				jsString = [result toSuccessCallbackString: callbackId];
			}
	...
	if (errorCode > 0) {
		...
		result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt: errorCode];
		jsString = [result toErrorCallbackString:callbackId];
	}
...
- (void) getParent:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if(newPath){
		...
		if (bExists) {
			result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsDictionary: [self getDirectoryEntry:newPath isDirectory:bIsDir]];
			jsString = [result toSuccessCallbackString:callbackId];
		}
		[fileMgr release];
	}
...
- (void) getMetadata:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (fileAttribs){
		...
		BOOL bIsDir;
		[fileMgr fileExistsAtPath: testPath isDirectory: &bIsDir];
        if(bIsDir == YES)
            size = 0;

		if (modDate){
            NSDictionary *metadata = [NSDictionary dictionaryWithObjectsAndKeys:
                                      [NSNumber numberWithDouble:[modDate timeIntervalSince1970]*1000]
                                      ,@"lastModified"
                                      ,[NSNumber numberWithUnsignedLongLong:size]
                                      ,@"size"
                                      , nil];
            result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary:metadata];
			jsString = [result toSuccessCallbackString:callbackId];
		}
	} else {
		// didn't get fileAttribs
		RTFileError errorCode = ABORT_ERR;
		...
		result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageAsInt: errorCode];
		jsString = [result toErrorCallbackString:callbackId];
	}
	if (jsString){
		[self writeJavascript:jsString];
...
- (void) setMetadata:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
    ...
    if (ok) {
		...
		result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR];
...
- (void) remove:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	RTPluginResult* result = nil;
	NSString* jsString = nil;
	RTFileError errorCode = 0;  // !! 0 not currently defined 
...
- (void) removeRecursively:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	// error if try to remove top level (documents or tmp) dir
	if ([fullPath isEqualToString:self.appDocsPath] || [fullPath isEqualToString:self.appTempPath]){
		result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: NO_MODIFICATION_ALLOWED_ERR];
		jsString = [result toErrorCallbackString:callbackId];
	} else {
		jsString = [self doRemove: fullPath callback: callbackId];
	}
...
- (NSString*) doRemove:(NSString*)fullPath callback: (NSString*)callbackId
{
	...
	@try {
		...
		result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: SYNTAX_ERR];
		jsString = [result toErrorCallbackString:callbackId];	
	}
	@finally {
		[fileMgr release];
...
- (void) doCopyMove:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options  isCopy:(BOOL)bCopy
{
    ...
    NSString* destRootPath = [[self getURIfromString:[arguments objectAtIndex:1]] path];
    // optional argument
	...
	RTPluginResult* result = nil;
	NSString* jsString = nil;
	RTFileError errCode = 0;  // !! Currently 0 is not defined, use this to signal error !!
		...
		if ( [newFullPath isEqualToString:srcFullPath] ){
			...
			if (!bSrcExists || !bDestExists) {
				...
				if (bSuccess) {
					...
					result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: newEntry];
					jsString = [result toSuccessCallbackString:callbackId];
				}
				else {
	...
	if (errCode > 0) {
		result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: errCode];
		jsString = [result toErrorCallbackString:callbackId];
	}
...
- (void) toURI:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (fullPath) {
		...
		if (fileUrl) {
			result = [PluginResult resultWithStatus:RTCommandStatus_OK messageAsString: [fileUrl absoluteString]];
			jsString = [result toSuccessCallbackString:callbackId];
		} // else NOT_FOUND_ERR
	}
	if(!jsString) {
		// was error
		result = [PluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: NOT_FOUND_ERR cast:  @"window.localFileSystem._castError"];
		jsString = [result toErrorCallbackString:callbackId];
	}
...
- (void) getFileMetadata:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (fullPath) {
		...
		if(!bExists || bIsDir){
			...
			result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: fileInfo];
			jsString = [result toSuccessCallbackString:callbackId];
		}
		[fileMgr release];
	}
...
- (void) readEntries:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (contents) {
		...
		result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsArray: entries];
		jsString = [result toSuccessCallbackString:callbackId];
	} else {
...
- (void) readAsText:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if(!file){
        ...
        result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString: [ pNStrBuff stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] ];
        jsString = [result toSuccessCallbackString:callbackId];
        [ pNStrBuff release ];
	...
	if(!argPath){
		...
		if (!mimeType) {
			...
			if (readData) {
				...
				result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsString: output];
				jsString = [result toSuccessCallbackString:callbackId];
			} else {
				errCode = NOT_FOUND_ERR;
			}
...
- (void) truncate:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt: newPos];
	[self writeJavascript:[result toSuccessCallbackString: callbackId]];
...
{	
	RTPluginResult* result = nil;
	NSString* jsString = nil;
	RTFileError errCode = INVALID_MODIFICATION_ERR; 
	int bytesWritten = 0;
	...
	if (filePath) {
		...
		if (fileStream) {
			...
			[ fileStream close ];
			if (bytesWritten > 0) {
				result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: bytesWritten];
				jsString = [result toSuccessCallbackString:callbackId];
			//} else {
...
- (void) testFileExists:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: ( bExists ? 1 : 0 )];
...
- (void) testDirectoryExists:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: ( (bExists && bIsDir) ? 1 : 0 )];
	// keep original format of returning 0 or 1 to success callback
	jsString = [result toSuccessCallbackString: callbackId];
	[fMgr release];
	[self writeJavascript: jsString];
...
- (void) getFreeDiskSpace:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsString: strFreeSpace];
	[self writeJavascript:[result toSuccessCallbackString: callbackId]];
