...
@implementation RTContactsPicker
...
@end
@implementation RTNewContactsController

@synthesize callbackId;

@end

@implementation RTContacts
...
{
    self = (RTContacts*)[super initWithWebView:(UIWebView*)theWebView];
    /*if (self) {
...
{	
	...
	RTNewContactsController* npController = [[[RTNewContactsController alloc] init] autorelease];
...
- (void) newPersonViewController:(ABNewPersonViewController*)newPersonViewController didCompleteWithNewPerson:(ABRecordRef)person
{
	...
	RTNewContactsController* newCP = (RTNewContactsController*) newPersonViewController;
	...
	RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt:  recordId];
...
- (void) displayContact:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (rec) {
		RTDisplayContactViewController* personController = [[[RTDisplayContactViewController alloc] init] autorelease];
	...
	else 
	{
		...
		RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt:  UNKNOWN_ERROR];
...
- (void) chooseContact:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	RTContactsPicker* pickerController = [[[RTContactsPicker alloc] init] autorelease];
	     ...
	     shouldContinueAfterSelectingPerson:(ABRecordRef)person
{
	...
	RTContactsPicker* picker = (RTContactsPicker*)peoplePicker;
	...
	if (picker.allowsEditing) {
		...
		// return the contact Id
		RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsInt: contactId];
...
- (void) peoplePickerNavigationControllerDidCancel:(ABPeoplePickerNavigationController *)peoplePicker
{
	...
	RTContactsPicker* picker = (RTContactsPicker*)peoplePicker;
	RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsInt: picker.selectedId];
	[self writeJavascript:[result toSuccessCallbackString:picker.callbackId]];
...
- (void) pickcontact:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (![findOptions isKindOfClass:[NSNull class]]){
		id value = nil;
		filter = (NSString*)[findOptions objectForKey:@"search"];
		value = [findOptions objectForKey:@"limit"];
        limit = [findOptions objectForKey:@"limit"];
        fields = [findOptions objectForKey:@"fields"];
    ...
    NSDictionary* returnFields = [[RTContact class] calcReturnFields: fields];
	...
	if (!filter || [filter isEqualToString:@""]){
		...
		if (foundRecords && [foundRecords count] > 0){
            ...
            int xferCount = multiple == NO ? [foundRecords count] : limit;
			matches = [NSMutableArray arrayWithCapacity:xferCount];
			for(int k = 0; k<xferCount; k++){
				RTContact* xferContact = [[[RTContact alloc] initFromABRecord:(ABRecordRef)[foundRecords objectAtIndex:k]] autorelease];
		...
		for(int j=0; j<testCount; j++){
			RTContact* testContact = [[[RTContact alloc] initFromABRecord: (ABRecordRef)[foundRecords objectAtIndex:j]] autorelease];
			if (testContact){
	...
	if (matches != nil && [matches count] > 0){
        ...
        int count = multiple == NO ? [matches count] : limit;
		for(int i = 0; i<count; i++){
			RTContact* newContact = [matches objectAtIndex:i];
    ...
    result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsArray: returnContacts];
...
- (void) add:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	BOOL bUpdate = NO;
	RTContactError errCode = UNKNOWN_ERROR;
	CFErrorRef error;
	RTPluginResult* result = nil;	
	...
	if (bSuccess){
		...
		if (!bSuccess){  // need to provide error codes
			...
			NSDictionary* newContact = [aContact toDictionary: [RTContact defaultFields]];
			//NSString* contactStr = [newContact JSONRepresentation];
			result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: newContact];
			jsString = [result toSuccessCallbackString:callbackId];
		}
	} else {
		bIsError = TRUE;
	...
	if (bIsError){
		result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageAsInt: errCode];
		...
		if (rec){
			...
			if (!bSuccess){
				...
				if(!bSuccess){
                    ...
                    //result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: contactDict];
 ...
 */ 
@implementation RTDisplayContactViewController
