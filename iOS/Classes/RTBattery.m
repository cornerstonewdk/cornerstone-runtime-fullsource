...
@interface RTBattery(PrivateMethods)
- (void) updateOnlineStatus;
@end

@implementation RTBattery
...
- (void) updateBatteryStatus: (NSNotification*)notification
{
    ...
    if (self.callbackId) {
        RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary:batteryData];
...
- (void) start:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
    ...
    if ( currentDevice.batteryMonitoringEnabled == NO) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateBatteryStatus:) 
												 name:UIDeviceBatteryStateDidChangeNotification object:currentDevice];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateBatteryStatus:) 
												 name:UIDeviceBatteryLevelDidChangeNotification object:currentDevice];
...
- (void) stop:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
    ...
    if (self.callbackId) {
        RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary:[self getBatteryStatus]];
...
{
    self = (RTBattery*)[super initWithWebView:theWebView];
    if (self) {
