...
//  NSString* chunkedMode = (NSString*)[arguments objectAtIndex:7]; // currently unused
    ...
    RTFileTransferError errorCode = 0;
    ...
    if(errorCode > 0) {
        result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: [self createFileTransferError:errorCode AndSource:filePath AndTarget:server]];
...
//    Magic value to set a cookie
	...
	RTFileTransferDelegate* delegate = [[[RTFileTransferDelegate alloc] init] autorelease];
...
- (void) download:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options {
    ...
    if(errorCode > 0) {
        result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: [self createFileTransferError:errorCode AndSource:sourceUrl AndTarget:filePath]];
...
@implementation RTFileTransferDelegate
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection 
{
    ...
    NSLog(@"File Transfer Finished with response  %@", self.responseData);
    ...
    if(self.direction == CDV_TRANSFER_UPLOAD)
    {
        if(self.responseCode >= 200 && self.responseCode < 300)
        {
            ...
            result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsDictionary: uploadResult];            
        } 
        else 
        {                  
            result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: [command createFileTransferError:CONNECTION_ERR AndSource:source AndTarget:target AndHttpStatus: self.responseCode]];
            errored = YES;
        }
    }
        ...
        if(self.responseCode >= 200 && self.responseCode < 300)
        {
            @try {
                ...
                if ( downloadResponse == NO ) {
                    ...
                    result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: [command createFileTransferError:INVALID_URL_ERR AndSource:source AndTarget:target AndHttpStatus: self.responseCode ]];
                    errored = YES;
                } else {
                    DLog(@"File Transfer Download success");
                    
                    file = [[[RTFile alloc] init] autorelease];
                    
                    result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsDictionary: [file getDirectoryEntry: target isDirectory: bDirRequest]];
                }
            }
            @catch (id exception) {
                // jump back to main thread
                result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: [command createFileTransferError:FILE_NOT_FOUND_ERR AndSource:source AndTarget:target AndHttpStatus: self.responseCode ]];
                errored = YES;
            }
        } else {
            result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: [command createFileTransferError:CONNECTION_ERR AndSource:source AndTarget:target AndHttpStatus: self.responseCode]];
...
{
    RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsDictionary: [command createFileTransferError: CONNECTION_ERR AndSource:source AndTarget:target AndHttpStatus: self.responseCode]];
