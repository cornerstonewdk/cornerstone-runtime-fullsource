...
static NSSet* org_skt_runtime_validArrowDirections;
...
+ (void) initialize
{
    org_skt_runtime_validArrowDirections = [[NSSet alloc] initWithObjects: [NSNumber numberWithInt:UIPopoverArrowDirectionUp], [NSNumber numberWithInt:UIPopoverArrowDirectionDown],[NSNumber numberWithInt:UIPopoverArrowDirectionLeft], [NSNumber numberWithInt:UIPopoverArrowDirectionRight], [NSNumber numberWithInt:UIPopoverArrowDirectionAny], nil];
}
...
- (void) takePicture:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
	...
	if (!hasCamera) {
		...
		RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString: @"no camera available"];
    ...
    if ([self popoverSupported] && sourceType != UIImagePickerControllerSourceTypeCamera)
    {
        ...
        if (options) {
            ...
            if(![org_skt_runtime_validArrowDirections containsObject:[NSNumber numberWithInt:arrowDirection]]){
...
- (void) popoverControllerDidDismissPopover:(id)popoverController
{
    ...
    if (self.pickerController && self.pickerController.callbackId && self.pickerController.popoverController) {
        ...
        RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString: @"no image selected"]; // error callback expects string ATM
	...
	if ([mediaType isEqualToString:(NSString*)kUTTypeImage])
	{
        ...
        if (cameraPicker.returnType == DestinationTypeFileUri) {
            ...
            if (![data writeToFile: filePath options: NSAtomicWrite error: &err]) {
                result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString: [err localizedDescription]];
                jsString = [result toErrorCallbackString:callbackId];
            } else {
                result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString: [[NSURL fileURLWithPath: filePath] absoluteString]];
                jsString = [result toSuccessCallbackString:callbackId];
            }
            [fileMgr release];
        
        } else {
            result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString: [data base64EncodedString]];
            jsString = [result toSuccessCallbackString:callbackId];
        }
	} 
