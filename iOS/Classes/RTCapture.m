...
#define kSRTCaptureFilepath @"destinationFilename"
#define kSRTCapturehighRes @"highRes"
...
@implementation RTCapture
@synthesize inUse;
@synthesize filepath;

-(id)initWithWebView:(UIWebView *)theWebView
{
	self = (RTCapture*)[super initWithWebView:theWebView];
	if(self)
	{
        self.inUse = NO;
    }
...
- (void) captureAudio:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
    ...
    NSString *file = [options objectForKey:kSRTCaptureFilepath];
    
    if([file hasPrefix:@"file://localhost"]) {
        file = [file stringByReplacingOccurrencesOfString:@"file://localhost" withString:@""];
    }
    
    if (NSClassFromString(@"AVAudioRecorder") == nil) {
        result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageToErrorObject: CAPTURE_NOT_SUPPORTED];
    }
    else if (self.inUse == YES) {
        result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageToErrorObject: CAPTURE_APPLICATION_BUSY];
    } else {
        // all the work occurs here
        RTAudioRecorderViewController* audioViewController = [[[RTAudioRecorderViewController alloc] initWithCommand:  self duration: duration callbackId: callbackId filepath:file] autorelease];
...
{
    NSString* callbackId = [arguments objectAtIndex:0];
    NSString* filename = [options objectForKey:kSRTCaptureFilepath];
    NSString* mode = nil;
    
    if([filename hasSuffix:@"png"]) {
        [mode initWithString:@"image/png"];
    } else {
        [mode initWithString:@"image/jpeg"];
    }
    
    if([filename hasPrefix:@"file://localhost"]) {
	...
	if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageToErrorObject:CAPTURE_NOT_SUPPORTED];
        [self writeJavascript:[result toErrorCallbackString:callbackId]];
...
-(NSString*) processImage: (UIImage*) image type: (NSString*) mimeType forCallbackId: (NSString*)callbackId
{
    ...
    if(![data writeToFile: filePath options: NSAtomicWrite error: &err]) {
        result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageToErrorObject: CAPTURE_INTERNAL_ERR ];
...
-(NSString*) processVideo: (NSString*) moviePath forCallbackId: (NSString*) callbackId
{
    ...
    result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString:moviePath];
    jsString = [result toSuccessCallbackString:callbackId];
...
- (void) getFormatData: (NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
    ...
    BOOL bError = NO;
    RTCaptureError errorCode = CAPTURE_INTERNAL_ERR;
    RTPluginResult* result = nil;
    NSString* jsString = nil;
    ...
    if (!mimeType){
        ...
        bError = !([command isKindOfClass:[RTFile class]]);
        if (!bError) {
            RTFile* rtfile = (RTFile*)command;
            mimeType = [rtfile getMimeTypeFromPath:fullPath];
            if (!mimeType) {
    ...
    if (!bError) {
        ...
        result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary:formatData];
        jsString = [result toSuccessCallbackString:callbackId];
        //NSLog(@"getFormatData: %@", [formatData description]);
    }
    if (bError) {
        result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageToErrorObject:errorCode];
        jsString = [result toErrorCallbackString:callbackId];
    }
    if (jsString) {
        [self writeJavascript:jsString];
...
-(NSDictionary*) getMediaDictionaryFromPath: (NSString*) fullPath ofType: (NSString*) type
{
    ...
    if(!type) {
        id command = [self.commandDelegate getCommandInstance: @"File"];
        if([command isKindOfClass:[RTFile class]]) {
            RTFile* rtfile = (RTFile*)command;
            NSString* mimeType = [rtfile getMimeTypeFromPath:fullPath];
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController*)picker
{
    ...
    NSString* jsString = nil;
    RTPluginResult* result = nil;
    result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageToErrorObject:CAPTURE_NO_MEDIA_FILES];
    jsString = [result toErrorCallbackString: callbackId];
...
@implementation RTAudioRecorderViewController
@synthesize errorCode, callbackId, duration, captureCommand, doneButton, recordingView, recordButton, recordImage, stopRecordImage, timerLabel, avRecorder, avSession, resultString, timer, isTimed, filepath;
...
- (id) initWithCommand:  (RTCapture*) theCommand duration: (NSNumber*) theDuration callbackId: (NSString*) theCallbackId filepath:(NSString *)filePath
{
    if ((self = [super init])) {
...
{	
    ...
    NSURL* fileURL = [NSURL fileURLWithPath:self.filepath isDirectory:NO];
    [fileMgr release];
...
- (void) dismissAudioView: (id) sender
{
    ...
    if (!self.resultString) {
        // return error
        RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageToErrorObject:self.errorCode];
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder*)recorder successfully:(BOOL)flag
{
    ...
    if (flag) {
        ...
        RTPluginResult* result = [RTPluginResult resultWithStatus: RTCommandStatus_OK messageAsString: [avRecorder.url path]];
        self.resultString = [result toSuccessCallbackString:callbackId];
    } else {
        RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageToErrorObject:CAPTURE_INTERNAL_ERR];
        self.resultString = [result toErrorCallbackString:callbackId];
    }
}
