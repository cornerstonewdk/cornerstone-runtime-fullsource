...
+ (BOOL) canInitWithRequest:(NSURLRequest *)theRequest
{
    ...
    if (gWhitelist == nil) {
        ...
        if ([delegate respondsToSelector:@selector(viewController)]) {
            ...
            if ([vc isKindOfClass:[RTViewController class]]) {
                gWhitelist = [((RTViewController*)vc).whitelist retain];
            }
        }
    }
...
- (id) initWithUnauthorizedURL:(NSURL*)url
{
    ...
    NSDictionary* headerFields = [NSDictionary dictionaryWithObject:@"Digest realm = \"SRT.plist/ExternalHosts\"" forKey:@"WWW-Authenticate"];
