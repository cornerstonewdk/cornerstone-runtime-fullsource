...
- (void) getLocation:(NSMutableArray *)arguments withDict:(NSMutableDictionary *)options
{
    ...
    if ([self isLocationServicesEnabled] == NO)
    {
        ...
        RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageAsDictionary:posError];
...
- (void)returnLocationInfo: (NSString*) callbackId andKeepCallback:(BOOL)keepCallback
{
    ...
    if (lData && !lData.locationInfo) {
        // return error
        result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageToErrorObject:POSITIONUNAVAILABLE];
        ...
        result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDictionary: returnInfo];
...
- (void)getHeading:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
    ...
    {
        RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageAsInt:20];
        [super writeJavascript:[result toErrorCallbackString:callbackId]];
    } else {
...
- (void)returnHeadingInfo: (NSString*) callbackId keepCallback: (BOOL) bRetain
{
    ...
    if (hData && hData.headingStatus == HEADINGERROR) {
        // return error
        result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageAsInt:0];
        jsString = [result toErrorCallbackString:callbackId];
...
- (void) startHeadingWithFilter: (CLLocationDegrees) filter
{
    if ([self.locationManager respondsToSelector: @selector(headingOrientation)]) {
        ...
        if (currentOrientation != UIDeviceOrientationUnknown) {
            RTViewController* rtViewController = (RTViewController*)self.viewController;
            
            if ([rtViewController.supportedOrientations containsObject:
	...
	if ([error code] == kCLErrorHeadingFailure)
	{
        ...
        if (hData) {
            if (hData.headingStatus == HEADINGSTARTING) {
                ...
                for (NSString* callbackId in hData.headingCallbacks) {
                    RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageAsInt:0];
                    [super writeJavascript: [result toErrorCallbackString:callbackId]];
                }
                [hData.headingCallbacks removeAllObjects];
            } // else for frequency watches next call to getCurrentHeading will report error
            if (hData.headingFilter) {
                RTPluginResult* resultFilter = [RTPluginResult resultWithStatus:RTCommandStatus_ERROR messageAsInt:0];
