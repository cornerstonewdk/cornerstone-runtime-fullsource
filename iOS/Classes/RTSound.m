...
- (NSURL*) urlForResource:(NSString*)resourcePath
{
    ...
    if ([resourcePath hasPrefix:HTTP_SCHEME_PREFIX] || [resourcePath hasPrefix:HTTPS_SCHEME_PREFIX]){
        ...
        filePath = [resourcePath stringByReplacingOccurrencesOfString:DOCUMENTS_SCHEME_PREFIX withString:[NSString stringWithFormat:@"%@/",[RTViewController applicationDocumentsDirectory]]];
...
{
	BOOL bError = NO;
	RTMediaError errcode = MEDIA_ERR_NONE_SUPPORTED;
    NSString* errMsg = @"";
	NSString* jsString = nil;
	RTAudioFile* audioFile = nil;
	NSURL* resourceURL = nil;
	...
	if (audioFile == nil){
		...
		} else if([resourcePath hasPrefix:@"file://localhost"]) {
            resourcePath = [resourcePath stringByReplacingOccurrencesOfString:@"file://localhost" withString:@""];
		...
		if (bError) {
			//jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, errcode];
            jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR,[self createMediaErrorWithCode: errcode message: errMsg]];
			[super writeJavascript:jsString];
		} else {
			audioFile = [[[RTAudioFile alloc] init] autorelease];
...
#pragma unused(callbackId)
	...
	RTAudioFile* audioFile = [self audioFileForResource:[arguments objectAtIndex:2] withId: mediaId];
	...
	if (audioFile != nil) {
		...
		if (!bError){
            ...
            if (!bError) {
                ...
                jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_DURATION, position, @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                [super writeJavascript:jsString];
        ...
        if (bError) {
			...
			//jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR,  MEDIA_ERR_NONE_SUPPORTED];
            jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_NONE_SUPPORTED message: nil]];
			[super writeJavascript:jsString];
		}
	}
...
- (BOOL) prepareToPlay: (RTAudioFile*) audioFile withId: (NSString*) mediaId
{
    ...
    if ([resourceURL isFileURL]) {
        audioFile.player = [[[ RTAudioPlayer alloc ] initWithContentsOfURL:resourceURL error:&playerError] autorelease];
    } else {
        ...
        if (playerError) {
            ...
            audioFile.player = [[[ RTAudioPlayer alloc ] initWithContentsOfURL:fileURL error:&playerError] autorelease];
        }
    }
...
- (void) prepare:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
    ...
    NSString* mediaId = [arguments objectAtIndex:1];
    BOOL bError = NO;
    RTMediaStates state = MEDIA_STARTING;
    NSString* jsString = nil;
    
	RTAudioFile* audioFile = [[self soundCache] objectForKey: mediaId];
    if (audioFile == nil) {
    ...
    if (!bError) {
        ...
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);\n%@", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_STATE, state, [result toSuccessCallbackString:callbackId]];
...
#pragma unused(callbackId)
	...
	if (audioFile != nil && audioFile.player!= nil) {
        ...
        [audioFile.player stop];
        audioFile.player.currentTime = 0;
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
...
#pragma unused(callbackId)
	...
	if (audioFile != nil && audioFile.player != nil) {
            ...
            jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_STATE, MEDIA_PAUSED];
	} 
    // ignore if no media playing
...
#pragma unused(callbackId)
    ...
    if (audioFile != nil && audioFile.player != nil && position){
        ...
        NSString* jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%f);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_POSITION, posInSeconds];
...
#pragma unused(callbackId)
	...
	RTAudioFile* audioFile = [[self soundCache] objectForKey: mediaId];
    double volume = [[arguments objectAtIndex:2 ] doubleValue];
	
    if (audioFile != nil && audioFile.player != nil &&volume){
        audioFile.player.volume = volume / 100;        
    }
...
#pragma unused(mediaId)
    ...
    RTPluginResult* result = [RTPluginResult resultWithStatus:RTCommandStatus_OK messageAsDouble: position];
	NSString* jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%.3f);\n%@", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_POSITION, position, [result toSuccessCallbackString:callbackId]];
    [super writeJavascript:jsString];
	...
	if (audioFile != nil) {
        ...
        if ([self hasAudioSession]) {
            ...
            if (![self.avSession  setActive: YES error: &error]){
                ...
                //jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, MEDIA_ERR_ABORTED];
                jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_ABORTED message: errorMsg] ];
                [super writeJavascript:jsString];
                return;
            }
        }
        ...
        audioFile.recorder = [[[RTAudioRecorder alloc] initWithURL:audioFile.resourceURL settings:nil error:&error] autorelease];
		...
		if (error != nil) {
            ...
            jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
		}
	} else {
        // file does not exist
        NSLog(@"Could not start recording audio, file '%@' does not exist.", audioFile.resourcePath);
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_ABORTED message: @"File to record to does not exist"]];
    }
    if (jsString) {
       [super writeJavascript:jsString]; 
    }
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder*)recorder successfully:(BOOL)flag
{
	...
	RTAudioRecorder* aRecorder = (RTAudioRecorder*)recorder;
    ...
    if (flag){
        ...
        //jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, MEDIA_ERR_DECODE];
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"srt.sktrequire('srt/plugin/Media').onStatuss", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_DECODE message:nil]];
    }
    if (self.avSession) {
        [self.avSession setActive:NO error:nil];
    }
...
{
	RTAudioPlayer* aPlayer = (RTAudioPlayer*)player;
    ...
    if (flag){
        ...
        //jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, MEDIA_ERR_DECODE];
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"srt.sktrequire('srt/plugin/Media').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_DECODE message:nil]];
...
@implementation RTAudioRecorder
@synthesize mediaId;
- (void) dealloc
{
    self.mediaId = nil;
