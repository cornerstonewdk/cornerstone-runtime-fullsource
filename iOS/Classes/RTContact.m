...
static NSDictionary*	org_skt_runtime_contacts_W3CtoAB = nil;
static NSDictionary*	org_skt_runtime_contacts_ABtoW3C = nil;
static NSSet*			org_skt_runtime_contacts_W3CtoNull = nil;
static NSDictionary*	org_skt_runtime_contacts_objectAndProperties = nil;
static NSDictionary*	org_skt_runtime_contacts_defaultFields = nil;

@implementation RTContact : NSObject
...
{
	if (org_skt_runtime_contacts_ABtoW3C == nil) {
		org_skt_runtime_contacts_ABtoW3C = [NSDictionary dictionaryWithObjectsAndKeys: 
		...
		[org_skt_runtime_contacts_ABtoW3C retain];
	}
	
	return org_skt_runtime_contacts_ABtoW3C;
}
+(NSDictionary*) defaultW3CtoAB
{
	if (org_skt_runtime_contacts_W3CtoAB == nil){
		org_skt_runtime_contacts_W3CtoAB = [NSDictionary dictionaryWithObjectsAndKeys: 
                ...
                [NSNumber numberWithInt: kABPersonFirstNameProperty],kW3ContactDisplayName, 
				...
				nil];
		[org_skt_runtime_contacts_W3CtoAB retain];
	}
	return org_skt_runtime_contacts_W3CtoAB;
}
+(NSSet*) defaultW3CtoNull
{
	
	// these are values that have no AddressBook Equivalent OR have not been implemented yet
	if (org_skt_runtime_contacts_W3CtoNull == nil){
		org_skt_runtime_contacts_W3CtoNull = [NSSet setWithObjects: kW3ContactDisplayName,
				kW3ContactCategories, kW3ContactFormattedName, nil];
	[org_skt_runtime_contacts_W3CtoNull retain];
	}
	return org_skt_runtime_contacts_W3CtoNull;
}
/*  
...
{
	if (org_skt_runtime_contacts_objectAndProperties == nil){
		org_skt_runtime_contacts_objectAndProperties = [NSDictionary dictionaryWithObjectsAndKeys: 
													 ...
													 nil];
		[org_skt_runtime_contacts_objectAndProperties retain];
	}
	return org_skt_runtime_contacts_objectAndProperties;
}
+(NSDictionary*) defaultFields
{
	if (org_skt_runtime_contacts_defaultFields == nil){
		org_skt_runtime_contacts_defaultFields = [NSDictionary dictionaryWithObjectsAndKeys:
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactName], kW3ContactName,
			[NSNull null], kW3ContactNickname,
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactAddresses], kW3ContactAddresses,
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactOrganizations], kW3ContactOrganizations,
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactPhoneNumbers], kW3ContactPhoneNumbers,
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactEmails], kW3ContactEmails,
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactIms], kW3ContactIms,
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactPhotos], kW3ContactPhotos,
			[[RTContact defaultObjectAndProperties] objectForKey: kW3ContactUrls], kW3ContactUrls,
			[NSNull null],kW3ContactBirthday,
			[NSNull null],kW3ContactNote,
			nil];
		[org_skt_runtime_contacts_defaultFields retain];

	}
	return org_skt_runtime_contacts_defaultFields;
}
...
+(void) releaseDefaults
{
	// ugly but it works
	if (org_skt_runtime_contacts_ABtoW3C != nil) {
		[org_skt_runtime_contacts_ABtoW3C release];
        org_skt_runtime_contacts_ABtoW3C = nil;
	}
	if (org_skt_runtime_contacts_W3CtoAB != nil) {
		[org_skt_runtime_contacts_W3CtoAB release];
        org_skt_runtime_contacts_W3CtoAB = nil;
	}
	if (org_skt_runtime_contacts_W3CtoNull != nil) {
		[org_skt_runtime_contacts_W3CtoNull release];
        org_skt_runtime_contacts_W3CtoNull = nil;
	}
    if (org_skt_runtime_contacts_objectAndProperties != nil) {
		[org_skt_runtime_contacts_objectAndProperties release];
        org_skt_runtime_contacts_objectAndProperties = nil;
	}
	if (org_skt_runtime_contacts_defaultFields != nil) {
		[org_skt_runtime_contacts_defaultFields release];
        org_skt_runtime_contacts_defaultFields = nil;
	}
}
...
-(bool) setFromContactDict:(NSMutableDictionary*) aContact asUpdate: (BOOL) bUpdate
{
	...
	if ([dict isKindOfClass:[NSDictionary class]]){
		bName = true;
		NSArray* propArray = [[RTContact defaultObjectAndProperties] objectForKey: kW3ContactName];
		for(id i in propArray){
			if (![(NSString*)i isEqualToString:kW3ContactFormattedName]){  //kW3ContactFormattedName is generated from ABRecordCopyCompositeName() and can't be set
				[self setValue:[dict valueForKey:i] forProperty: (ABPropertyID)[(NSNumber*)[[RTContact defaultW3CtoAB] objectForKey: i]intValue] 
	...
	if (!bName){
		...
		[self setValue:[aContact valueForKey:kW3ContactDisplayName] forProperty: kABPersonFirstNameProperty 
						 inRecord: person asUpdate: bUpdate];
	}
...
-(bool) addToMultiValue: (ABMultiValueRef) multi fromDictionary: dict{
	...
	if (IS_VALID_VALUE(value)){
		NSString* label = (NSString*)[RTContact convertContactTypeToPropertyLabel:[dict valueForKey:kW3ContactFieldType]];
	...
	if (!bUpdate){
		...
		if (copy != nil){
			...
			for(NSDictionary* dict in fieldArray){
				...
				label = (NSString*)[RTContact convertContactTypeToPropertyLabel:[dict valueForKey:kW3ContactFieldType]];
...
-(NSMutableDictionary*) translateW3Dict: (NSDictionary*) dict forProperty: (ABPropertyID) prop
{
	...
	NSArray* propArray = [[RTContact defaultObjectAndProperties] valueForKey: [[RTContact defaultABtoW3C] objectForKey:[NSNumber numberWithInt: prop]]];
	...
	for(NSString* key in propArray){ // for each W3 Contact key get the value
		if ((value = [dict valueForKey:key]) != nil && ![value isKindOfClass:[NSNull class]]){
			...
			if ([RTContact needsConversion: key]){ // IM types must be converted
				setValue = (NSString*)[RTContact convertContactTypeToPropertyLabel:value];
			...
			[newDict setObject:setValue forKey: (NSString*)[[RTContact defaultW3CtoAB] valueForKey:(NSString*)key]];
		}
	}
	if ([newDict count] ==0){
		...
		if (copy) {
			...
			for (NSDictionary* field in array) {
				...
				if (idx != kCFNotFound){
                    ...
                    {
                        CFStringRef w3cLabel = [RTContact convertContactTypeToPropertyLabel:testLabel];
                    ...
                    for (id k in propArray){
						...
						if (bSet){  
							NSString* setValue = [RTContact needsConversion:(NSString*)k] ? (NSString*)[RTContact convertContactTypeToPropertyLabel:value] : value;
							[dict setObject:setValue forKey: (NSString*)[[RTContact defaultW3CtoAB] valueForKey:(NSString*)k]];
							...
							valueAB = [existingDictionary valueForKey:[[RTContact defaultW3CtoAB] valueForKey:k]];
							if (valueAB != nil){
								[dict setValue:valueAB forKey:[[RTContact defaultW3CtoAB] valueForKey:k]];
							}
...
-(NSObject*) extractName
{
	...
	for (NSString* i in fields){
		if ([i isEqualToString:kW3ContactFormattedName]){
			...
			value = [(NSString*)ABRecordCopyValue(self.record, (ABPropertyID)[[[RTContact defaultW3CtoAB] valueForKey:i] intValue]) autorelease];
...
-(NSObject*) extractMultiValue: (NSString*)propertyId
{
	...
	NSNumber* propNumber = [[RTContact defaultW3CtoAB] valueForKey:propertyId];
	...
	if (count){
		...
		for (CFIndex i=0; i<count; i++) {
			...
			if ([fields containsObject:kW3ContactFieldType]){
				...
				value = [RTContact convertPropertyLabelToContactType: label];
...
-(NSObject*) extractAddresses 
{
	...
	for (CFIndex i = 0; i < count; i++) {
        ...
        [newAddress setObject: (label != nil) ? (NSObject*) [[RTContact class] convertPropertyLabelToContactType:label] : [NSNull null] forKey:kW3ContactFieldType];
		...
		for(id k in fields){
			bool bFound;
			id key = [[RTContact defaultW3CtoAB] valueForKey:k];
	...
	if (count){
		...
		for (CFIndex i = 0; i < ABMultiValueGetCount(multi); i++) {
			...
			if ([fields containsObject: kW3ContactFieldType]){
				...
				[newDict setObject: (bFound && value != NULL) ? (id)[[RTContact class ]convertPropertyLabelToContactType: value] : [NSNull null] forKey: kW3ContactFieldType];
			}
			// always set ID
			id identifier = [NSNumber numberWithUnsignedInt: ABMultiValueGetIdentifierAtIndex(multi,i)];
...
-(NSObject*) extractOrganizations
{
	...
	for (id i in fields){
		id key = [[RTContact defaultW3CtoAB] valueForKey:i];
...
{	//NSLog(@"getting self.returnFields");
	...
	if (fieldsArray != nil && [fieldsArray isKindOfClass:[NSArray class]]){
        if ([fieldsArray count] == 1 && [[fieldsArray objectAtIndex:0] isEqualToString:@"*"]) {
            return [RTContact defaultFields];  // return all fields
        }
		for (id i in fieldsArray){
			...
			id fields = [[RTContact defaultObjectAndProperties] objectForKey:name];
			...
			if (fields != nil && property == nil){ //request was for full object
				...
				id abEquiv = [[RTContact defaultW3CtoAB] objectForKey:property];
				if (abEquiv || [[RTContact defaultW3CtoNull] containsObject:property]){
				...
				id valid = [[RTContact defaultW3CtoAB] objectForKey:name];
				if (valid || [[RTContact defaultW3CtoNull] containsObject:name]){
...
-(BOOL) foundValue: (NSString*)testValue inFields: (NSDictionary*) searchFields
{
	...
	if (!bFound && [searchFields valueForKey:kW3ContactDisplayName]){
		bFound = [self testStringValue:testValue forW3CProperty:kW3ContactDisplayName];
	} 
...
-(BOOL) testStringValue: (NSString*)testValue forW3CProperty: (NSString*) property {
	...
	if ([[RTContact defaultW3CtoAB] valueForKeyIsNumber: property ]) {
		ABPropertyID propId = [[[RTContact defaultW3CtoAB] objectForKey: property] intValue];
...
-(BOOL) searchContactFields: (NSArray*) fields forMVStringProperty: (ABPropertyID) propId withValue: testValue {
	...
	for (NSString* type in fields){
		...
		if ([type isEqualToString: kW3ContactFieldType]){
			if ([RTContact isValidW3ContactType: testValue]){
				// only search types if the filter string is a valid ContactField.type
				testString = (NSString*)[RTContact convertContactTypeToPropertyLabel:testValue];
			}
		} else {
...
-(BOOL) searchContactFields: (NSArray*) fields forMVDictionaryProperty: (ABPropertyID) propId withValue: (NSString*)testValue
{
	...
	for(id dict in values){
		for(NSString* member in fields){
			NSString* abKey = [[RTContact defaultW3CtoAB] valueForKey:member]; // im and address fields are all strings
