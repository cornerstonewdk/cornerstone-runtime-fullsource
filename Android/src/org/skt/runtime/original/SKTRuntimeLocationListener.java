...
public class SKTRuntimeLocationListener implements LocationListener {
    ...
    private String TAG = "[SKTRuntime Location Listener]";
	
    public SKTRuntimeLocationListener(LocationManager manager, GeoBroker broker, String tag) {
	...
	public void onStatusChanged(String provider, int status, Bundle extras) {
		...
		if (status == 0) {
			...
			this.fail(SKTRuntimeLocationListener.POSITION_UNAVAILABLE, "Provider " + provider + " is out of service.");
		}
		else if (status == 1) {
	...
	protected void start() {
		if (!this.running) {
			if (this.locationManager.getProvider(LocationManager.NETWORK_PROVIDER) != null) {
				...
				this.fail(SKTRuntimeLocationListener.POSITION_UNAVAILABLE, "Network provider is not available.");
			}
		}
	}
